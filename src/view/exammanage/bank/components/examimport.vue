<template>
    <div>
        ...
        <Modal v-model="show" title="题库导入"
            :mask-closable="false"  :closable="false">
             <Form ref="modalForm" :model="data" :rules="ruls"
                    :label-width="110">
                <FormItem label="题库名称：" prop='bankname'>
                    <Input v-model.trim="data.bankname" type="text"></Input>
                </FormItem>
                <FormItem label="导入题库：" prop='fileName'>
                    ...
                    <div class="ivu-form-item-error-tip" v-show='noImportFile'>请导入题库列表</div>
                </FormItem>
            </Form>
            <div slot="footer">
...
export default {
    ...
    methods:{
        ...
        async resetPost(){
            ...
            try {
                let res = await post("/exam/manager/bank/import?bankname="+this.data.bankname,formdata,
                )
                this.cancel(true)
            } catch (error) {
                this.$throw(error)
